...
void get_line(char* str, FILE* pFile){
    ...
    // get every line just like coding
    do{
...
void random_space(FILE* pFile){
    ...
    for(int32_t i = 0;i < times;i++){
        ...
        int32_t choice = rand() % 50;

        if(choice < 49) fwrite(&space,sizeof(char),1,pFile);
...
int main(int argc, char *argv[]){
    ...
    // operator found , input_file open success , output_file open success
    {
    ...
    // open files
        ...
        if((tmp_file = fopen("tmp.c","wb+")) == NULL){  // temp_file 
            perror("Error");
            return 0;
        }

        while((c = fgetc(input_file)) != -1) {  fputc(c,tmp_file);  } 
        // save input into temp_file in order not to break the original file
        // save the edited content to temp_file ( from head )
